...
def get_query_today(message):
    try:
        ...
        queries = (make_query(f'''select count(*) from (select * from Queries where Time_d = {Time_d}); '''))[0][0]
        queries_s = (make_query(f'''select count(*) from (select * from Queries_stat where Time_d = {Time_d}); '''))[0][0]
        ...
        report_error(e)
    try:
        query_add_stat(message, take_phrase_2('stat_db', 'requests_today', 0), answer)
        check_user(message)
...
def get_query_all(message): 
    try:
        ...
        report_error(e)
    try:
        query_add_stat(message, take_phrase_2('stat_db', 'requests_all', 0), answer)
        check_user(message)
        check_user_all(message)
    except Exception as e:
        report_error(e)
